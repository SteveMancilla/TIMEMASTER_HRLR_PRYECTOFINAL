...
from src.modelo.db import DB, PomodoroModel, UsuarioModel
...
class Pomodoro(QMainWindow):
    ...
    def __init__(self):
        ...
        if self.usuario is None:
                messagebox.showerror("Error", "No se encontro ningun usuario registrado")
        
        self.btnIniciarPom.clicked.connect(self.guardarPomoDB)
        
    
    def guardarPomoDB(self):
        try:
                self.tiempoTrabajo = self.cbMinutosTrabajoPom.currentText()
                self.tiempoDescanso = self.cbMinDescansoPom.currentText()
                self.user = self.usuario.Usuario_ID
                
                pomodoro = PomodoroModel(
                        temPom_Duracion_trabajo = self.tiempoTrabajo,
                        temPom_Duracion_descanso = self.tiempoDescanso,
